 ...
 * MIT License
 *
 * modified by s.butterfill --- animates backgroundPosition instead of top and left 
 *
 */
        ...
        $.crSpline.buildSequence = function(pointList) {
                ...
                res.getPos = function (t) {
                        ...
                        if (segNum === numSegments) {
                                return {
                                        backgroundPositionX: seq[seq.length-2][0],
                                        backgroundPositionY: seq[seq.length-2][1]
                                        };
                        }
                        var microT = (t - segNum/numSegments) * numSegments;
                        var result = {
                                backgroundPositionX: interpolate(microT,
                                                seq[segNum][0],
                                                seq[segNum+1][0],
                                                seq[segNum+2][0],
                                                seq[segNum+3][0]) + "px",
                                backgroundPositionY: interpolate(microT,
                                                seq[segNum][1],
                                                seq[segNum+1][1],
                                                seq[segNum+2][1],
                                                seq[segNum+3][1]) + "px"
